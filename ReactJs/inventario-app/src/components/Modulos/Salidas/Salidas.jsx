...
const Salidas = () => {
  ...
  // Se define el estado "salidas" que inicialmente es un arreglo vacío. 
  ...
  // Función para cargar los datos de salidas
  const handleAdd = async () => {
    try {
      const range = 'Salidas!A:F';
  ...
  const totalPages = Math.max(1, Math.ceil(salidas.length / rowsPerPage));

  // Obtener los datos a mostrar según la página seleccionada
  const paginatedRows = salidas.slice(
  ...
  return (
    <div>
      ...
      {/* Modales */}
      {isAgregarOpen && <Agregar type="salida" closeModal={closeAgregarModal} onAdd={handleAdd} />}
      {isModificarOpen && <Modificar type="salida" closeModal={closeModificarModal} onUpdate={handleDataUpdate} />}
      {isEliminarOpen && <Eliminar type="salida" closeModal={closeEliminarModal} onUpdate={handleDataUpdate} />}

      {/* Pdf Modal */}
      {isPdfOpen && <PdfModal open={isPdfOpen} onClose={() => setIsPdfOpen(false)} data={salidas} type="salidas" />}
    </div>
  );
};
