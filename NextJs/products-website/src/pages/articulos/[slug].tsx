...
// Genera de forma estática todas las rutas dinámicas (/articles/[slug])
// a partir de los slugs definidos en el arreglo `articles`
export const getStaticPaths: GetStaticPaths = async () => {
  return {
    paths: articles.map((article) => ({
      params: { slug: article.slug }, // Cada slug crea una página
    })),
    fallback: false, // Solo se permiten las rutas generadas, las demás dan 404
  };
};

// Obtiene los datos del artículo para cada página generada
export const getStaticProps: GetStaticProps = async ({ params }) => {
  // Busca el artículo que coincida con el slug de la URL
  const article = articles.find(
    (article) => article.slug === params?.slug
  );

  // Si no existe el artículo, muestra la página 404
  if (!article) {
    return { notFound: true };
  }

  // Envía el artículo como props al componente de la página
  return {
    props: { article },
  };
};